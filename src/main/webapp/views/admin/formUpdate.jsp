	...
	<div class="container-fluid">
		...
		<div class="column-main column-right">
			...
			<div class="container mt-5" style="width: 70%">
				...
				<form action="crud?status=update" method="post" class="mt-5">
					<div class="form-floating mb-3 mt-3">
						<input type="text" class="form-control" id="email"
							placeholder="Enter email" value="${p.id }" name="masp"> <label
							for="email">Mã sản phẩm</label>
					</div>
					<div class="form-floating mt-3 mb-3">
						<input type="text" class="form-control" id="pwd"
							placeholder="Enter password" name="tensp" value="${p.ten }"> <label
					...
					<div class="form-floating mt-3 mb-3">
						<input type="text" class="form-control" id="pwd"
							placeholder="Enter password" name="mota" value="${p.mota }"> <label
							for="pwd">Mô tả</label>
					</div>
					<div class="form-floating mt-3 mb-3">
						<input type="text" class="form-control" id="pwd"
							placeholder="Enter password" name="dongia" value="${p.dongia }"> <label
							for="pwd" >Đơn giá</label>
					</div>
					<select class="form-select" name="danhmuc">
						<c:forEach var="categories" items="${categories }">
							<c:if test="${p.danhmuc == categories.ten}">
								<option selected>${categories.ten }</option>
							</c:if>
							<c:if test="${p.danhmuc != categories.ten}">
					...
					<div class="form-floating mt-3 mb-3">
						<input type="text" class="form-control" id="pwd"
							placeholder="Enter password" name="soluong" value="${p.soluong }"> <label
